...
```
To further optimize the model, we have investigated on a few feature selection algorithms such as recursive feature elimination (RFE) and Boruta algorithm. However, as it is found that the running time for these feature selection algorithm takes too long, that it takes more than 2 hours to tune the hyperparameters with cross validation. Thus we decided not to apply any feature selection algorithm at this stage.  
