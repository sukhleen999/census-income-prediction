...
As all the columns in the test data are of object type, we need to change the data types of some of the features like `age`, `fnlwgt` to the numeric columns, to make it suitable for testing purposes afterwards

## Analysis

In the Exploratory Data Analysis (EDA), we would like to assess the importance of each feature towards the prediction of the income level. We are going to visualize the the distribution of features, and see if there is potential bias in the data set so that we could carry out suitable processing pipeline to different features.

### Visualising numerical features

#### Distribution

We are starting with numeric features. Here we are visualizing the distribution of each numeric feature of each target class. The blue color represents the group with annual income \<=50K USD, where the orange color represents the counterpart.
...
From the plots above, we can see that the features `age`, `education_num`, `hours_per_week` are the major features which are demarcating the difference between the 2 classes clearly
